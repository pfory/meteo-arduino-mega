...
namespace SolarSystem
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void calculateSolarPower() {

        }

        private float getDopoctenaHodina()
        {
            return DateTime.Now.Hour + DateTime.Now.Minute / 60 + DateTime.Now.Second / 3600 + (sd.zemepisnaDelka / 15 - 1);
        }

        private float getDeklinace() {
            return (float)(23.45 / 180 * Math.PI * Math.Sin(Math.PI * (0.98 / 180 * DateTime.Now.Day + 29.7 / 180 * DateTime.Now.Month - 109 / 180)));
        }

        private float getDenniUhel()
        {
            return (float)(Math.PI/6*(getDopoctenaHodina()-12));
        }

        private float vyskaNadObzorem()
        {
            return (float)(Math.Asin(Math.Sin(getDeklinace())*Math.Sin(sd.zemepisnaDelka)+Math.Cos(getDeklinace()*Math.Cos(sd.zemepisnaDelka)*Math.Cos(getDenniUhel())));
        }
    ...
    public class SolarData : INotifyPropertyChanged
    {
        ...
        public float solarPower { get { return _solarPower; } 
            set { _solarPower = (5f / 1000f / 60f) * (_solarOutput - _solarInput) * 1054f * 3900f; NotifyPropertyChanged("solarPower");
        ...
        public Visibility pictureClouds { get { return _pictureClouds; } 
            set {
                if (solarPower > 0)
        ...
        public Visibility pictureSun { get { return _pictureSun; }
            set {
                if (solarPower > 0)
